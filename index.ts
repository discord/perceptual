...
function convertSliderPosition(pos: number, identifier: string): number {
    ...
    if (identifier === '3') {
        return perceptualToAmplitude(pos, 100, 30) * ampScale;
    }
    return 0;
}
...
(()=> {
    ...
    for (const d in demos) {
        ...
        demos[d].volume.addEventListener('input', () => {
            ...
            demos[d].labelAmplitude.innerText = `Amplitude: ${(amp / ampScale).toFixed(3)}% (${ampToDB((amp / ampScale) / 100).toFixed(1)} dB)`;
            if (state === `running-${d}`) {
